---
layout: page
title: Publications
excerpt: "A List of Publications"
---



*| [**Markov Random Field based Small Obstacle Discovery over Images.**](/docs/Suryansh_etal_ICRA_14.pdf" target="_blank)   | 
 | Suryansh Kumar, *Siva Karthik M*, K Madhava Krishna.   |
 | International Conference on Robotics and Automation(ICRA), 2014, Hong Kong.| 


*| [**Small Object Discovery and Recognition using Actively Guided Robots.**](/docs/Sudhanshu_etal_ICPR_14.pdf" target="_blank)   | 
 |  Sudhanshu Mittal<sup>*</sup>, <em>Siva Karthik M</em><sup>*</sup>, Suryansh Kumar and K.Madhava Krishna.   |
 | International Conference on Pattern Recongnition, 2014, Sweden.| 


*| [**Guess from Far, Recognize when Near: Searching Floor for Small Objects.**](/docs/Siva_etal_ICVGIP_14.pdf" target="_blank)   | 
 | *Siva Karthik M*, Sudhanshu Mittal and K.Madhava Krishna.  |
 | Indian Conference on Vision, Graphics and Image Processing, 2014, Bangalore.| 


 <sup>*</sup> Equal Contribution.
